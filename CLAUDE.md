...
> Master reference. Read this FIRST. Reflects the real built plugin (v0.7.18).
...
| `_wcpp_priority` | integer 1–99 (default 10); lower = higher priority when sets overlap |
...
- Review total is quantity-aware and respects the set fee type (`one-time` vs `per-item`).
...
- **Add-to-cart redirect filter** — the AJAX response URL uses
  `apply_filters('woocommerce_add_to_cart_redirect', wc_get_cart_url(), $product)` so
  sites configured to skip the cart and go to checkout behave consistently.
  `$product` (the WC_Product object) is passed as the second arg, matching WC core behaviour.
...
cached file and your change appears to "do nothing". Current: 0.7.18.
